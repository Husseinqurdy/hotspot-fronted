...
export default function SettingsModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // Fetch fresh every time the modal opens; reset local edit state when closed
  useEffect(() => {
    if (open) {
      fetchProfile()
      setEditing(false)
  ...
  return createPortal(
    <>
      <style>{`
        ...
        @keyframes settingsCardIn { from{opacity:0;transform:translateY(12px) scale(0.98)} to{opacity:1;transform:translateY(0) scale(1)} }
      `}</style>

      {/* Overlay */}
      <div onClick={onClose} style={{
        position: 'fixed', inset: 0, background: 'rgba(15,15,25,0.5)',
        zIndex: 1000, animation: 'settingsOverlayIn 0.16s ease',
        ...
        <div onClick={e => e.stopPropagation()} style={{
          ...
          animation: 'settingsCardIn 0.2s cubic-bezier(0.22,1,0.36,1)',
        }}>
          {/* Header */}
      ...
      {toast && (
        <div style={{
          ...
          boxShadow: '0 8px 20px rgba(0,0,0,0.15)', zIndex: 1100,
        }}>
          {toast.msg}
        </div>
      )}
