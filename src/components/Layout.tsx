  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button style={btnStyle} onClick={() => setOpen(o => !o)}
        ...
        <Icon.Globe />
        <span style={{ fontSize: 11, letterSpacing: '0.05em' }}>{current.short}</span>
      ...
      {open && (
        <div style={{ position: 'absolute', top: 'calc(100% + 6px)', right: 0, background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden', zIndex: 200, boxShadow: '0 8px 28px rgba(0,0,0,0.12)', minWidth: 160, animation: 'dropDown 0.18s ease' }}>
          {LANGS.map(l => (
...
function TopBar({ sidebarW, displayName, isAdmin, initials }: { sidebarW: number; displayName: string; isAdmin: boolean; initials: string }) {
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: sidebarW, right: 0, height: 60, background: '#fff', borderBottom: '1px solid #e5e7eb', zIndex: 30, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 1.5rem', boxShadow: '0 1px 8px rgba(0,0,0,0.06)', transition: 'left 0.25s ease', animation: 'topbarSlide 0.35s ease' }}>

      {/* Left - greeting + page title */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <h2 style={{ fontSize: 16, fontWeight: 800, color: '#111827', margin: 0, letterSpacing: '-0.3px' }}>
            {t('welcome_client')}, <span style={{ color: '#6366f1' }}>{displayName}</span>
          </h2>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <div style={{ width: 5, height: 5, borderRadius: '50%', background: '#6366f1', opacity: 0.5 }} />
          <span style={{ fontSize: 11, color: '#9ca3af', fontWeight: 500 }}>{getTitle()}</span>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div ref={notifRef} style={{ position: 'relative' }}>
          <button onClick={() => { setNotifOpen(o => !o); setProfileOpen(false) }}
            style={{ width: 38, height: 38, borderRadius: 10, border: '1px solid #e5e7eb', background: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#6b7280', transition: 'all 0.18s', position: 'relative' }}
          ...
          {notifOpen && (
            <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, background: '#fff', border: '1px solid #e5e7eb', borderRadius: 14, width: 300, zIndex: 200, boxShadow: '0 8px 28px rgba(0,0,0,0.12)', overflow: 'hidden', animation: 'dropDown 0.18s ease' }}>
        ...
        <div ref={profileRef} style={{ position: 'relative' }}>
          <button onClick={() => { setProfileOpen(o => !o); setNotifOpen(false) }}
            style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 10px 5px 5px', borderRadius: 12, border: '1px solid #e5e7eb', background: '#f8fafc', cursor: 'pointer', transition: 'all 0.18s' }}
            ...
            <div style={{ color: '#9ca3af', marginLeft: 2 }}><Icon.ChevronDown /></div>
          </button>

          {profileOpen && (
            <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, background: '#fff', border: '1px solid #e5e7eb', borderRadius: 14, width: 220, zIndex: 200, boxShadow: '0 8px 28px rgba(0,0,0,0.12)', overflow: 'hidden', animation: 'dropDown 0.18s ease' }}>
              {/* Profile header */}
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const Sidebar = () => (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      {/* Bottom - only lang switcher in sidebar (compact) */}
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f1f5f9' }}>
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .layout-topbar { left: 0 !important; }
      ...
      <div className="mobile-menubar">
        <button onClick={() => setMobileOpen(true)} style={{ width: 38, height: 38, borderRadius: 10, background: '#1e1b4b', border: 'none', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 10px rgba(0,0,0,0.2)' }}>
          <Icon.Menu />
        </button>
      </div>
      ...
      {mobileOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, animation: 'overlayIn 0.2s ease' }}>
          ...
          <div style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: 240, background: 'linear-gradient(180deg, #1e1b4b 0%, #13103a 100%)', animation: 'sidebarIn 0.25s ease', boxShadow: '4px 0 24px rgba(0,0,0,0.3)' }}>
            <Sidebar />
          </div>
        </div>
      )}
